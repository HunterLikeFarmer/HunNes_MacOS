...
// This means the CPU state snapshot at a certain timestamp. CPU state is represented by its values in all registers
...
class CPU6502 {
    ...
    // The status register is 8 bits, so each bit represent a status
    enum StatusFlags {
        NEGATIVE = 7,
        OVERFLO = 6,
        BREAK5 = 5,
   ...
   public:
    ...
    // Access different region of the memory with different r/w mode
   ...
   private:
    ...
    int cycle = 7;  // reset() takes 7 cycles to finish (to do with the pipeline stuff?)
    ...
    // These are external event function. In hardware these represent pins to produce a change in state
    inline void irq();  
    // Non-maskable interrupt. It works the same way as IRQ but reads the new PC from $FFFA
    inline void NMI();
