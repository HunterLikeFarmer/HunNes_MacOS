...
u8 PPU::ppuread(u16 address) {
    if (address >= 0x0000 && address <= 0x1FFF) {
        ...
        else if (mapper->getMirroring() == 1) { // Vertical
            if (address >= 0x2800 && address < 0x3000) {
                address -= 0x800;
            }
        }
        else if (mapper->getMirroring() == 2) { // One-screen mirroring, lower-bank (MMC1)
            address &= ~0xC00;
        }
        else { // One-screen mirroring, upper-bank (MMC1)
            address = (address & ~0xC00) + 0x400;
        }
        return vram[address - 0x2000];
    }
    else if (address >= 0x3F00 && address <= 0x3F0F) {
        if (address == 0x3F04 || address == 0x3F08 || address == 0x3F0C) {
            address = 0x3F00;
        }
        return bg_palette[address - 0x3F00];
    }
    else if (address >= 0x3F10 && address <= 0x3F1F) {
...
void PPU::evalSprites() {
    ...
    if (dot >= 257 && dot <= 320) {
        ...
        if (cycle >= 0 && cycle <= 1) {
            ...
            // Nothing happens for cycle 5; just retain the behavior
        }
        else if (cycle == 6) {
            if (!isUninit(sprite)) {
            ...
            // Default behavior for any other cycle value
        }
